...
export function SettingsMenu({ onHelp }: SettingsMenuProps) {
  ...
  const { viewType, setViewType, userMode, selectedEntity } = useScheduleStore();
  ...
  useEffect(() => {
    if (userMode === 'teacher' && selectedEntity?.toUpperCase().includes('MAGGIORE')) {
      if (theme !== 'light') {
        setTheme('light');
      }
    }
  ...
  return (
    <div className={styles.menuWrapper} ref={ref}>
      <button className={styles.iconBtn} title="Impostazioni" onClick={() => setOpen(o => !o)}>⚙️</button>
      {open && (
        <div className={styles.menu}>
          <div className={styles.groupLabel}>Aspetto</div>
          <button className={styles.item} onClick={cycleTheme} disabled={userMode === 'teacher' && selectedEntity?.toUpperCase().includes('MAGGIORE')}>
            <span className={styles.row}>🌓 Tema</span>
            <span className={styles.badge}>{userMode === 'teacher' && selectedEntity?.toUpperCase().includes('MAGGIORE') ? 'Chiaro fisso' : themeLabel}</span>
          </button>
          <div className={styles.groupLabel}>Vista</div>
          <button className={styles.item} onClick={() => setViewType(viewType === 'list' ? 'timeline' : 'list')}>
            <span className={styles.row}>👁️ Modalità</span>
            <span className={styles.badge}>{viewType === 'list' ? 'Lista' : 'Timeline'}</span>
          </button>
          <div className={styles.groupLabel}>Aiuto</div>
          <button className={styles.item} onClick={() => { onHelp(); setOpen(false); }}>
